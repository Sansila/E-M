...
class Category extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->thead = array(
			"No"=>'no',
			"Category Name"=>'location_name',	
			"Category Type"=>'location_type',	
			"Visibled"=>'visibled',
			"Action"=>'Action'			
		);
	...
	function getdata()
	{
		...
		$sql="SELECT location_id, location_name, location_type, is_active 
		FROM tbllocation
    ...
    $this->green->setActivePage($this->input->post('p')); 
		...
		foreach($this->db->query($sql)->result() as $row){
			...
			$table.= "<tr>
				...
				<td class='type'>".$row->location_type."</td>
	...
	function save()
	{
		...
		if($count>0){
			...
			$menu_id=$this->cat->save($location_id,$location_name,$location_type,$is_active);
