...
class pagepermissionmodel extends CI_Model {
	...
	{	
		...
		$config['num_link']=3;
		$this->db->select('*');
		$this->db->from('z_role r');
		$this->db->join('z_role_page rp','r.roleid=rp.roleid','inner');
		$this->db->join('z_page p','rp.pageid=p.pageid','inner');
	...
	function getpagepermisrow($id){
		...
		$query=$this->db->get('z_role_page');
		return $query->row();
	}
	function getpagebymodule($moduleid){
		$this->db->where('moduleid',$moduleid);
		$this->db->where('is_active',1);
		$query=$this->db->get('z_page');
		return $query->result();
	}
	function getmodulebyrole($roleid){
		$this->db->select('m.moduleid,m.module_name');
		$this->db->from('z_role r');
		$this->db->join('z_role_module_detail rm','r.roleid=rm.roleid','inner');
		$this->db->join('z_module m','rm.moduleid=m.moduleid','inner');
