							...
							<table class="table table-bordered table-striped table-hover">
								<thead>
									...
									<tr class='remove_tag'>
										...
										<th >
											<select id='s_storeid' class='form-control' onchange="getdata(1);">
												<option value=''>----select---</option>
												<?php foreach ($this->db->query("SELECT * FROM tbllocation WHERE is_active='1' AND location_type='banner'")->result() as $me) {
...
<script type="text/javascript">
	...
	function getdata(page){
		var url="<?php echo site_url('setup/setupads/getdata')?>";
		var m="<?PHP echo $m?>";
		var p="<?PHP echo $p?>";
	...
	function update(event){
		var adsid=jQuery(event.target).attr("rel");
		location.href="<?PHP echo site_url('setup/setupads/edit');?>/"+adsid+"?<?php echo "m=$m&p=$p" ?>";
	...
	function deletestore(event){
		...
		if(conf==true){
			var storeid=jQuery(event.target).attr("rel");
			var url="<?php echo site_url('setup/setupads/delete')?>/"+storeid;
			$.ajax({
				url:url,
				type:"POST",
				datatype:"Json",
