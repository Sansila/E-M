...
<div id="breadcrumb">
  ...
  <a href='#' class="current"><?php if(isset($row->location_id)) echo 'Edit Menu'; else echo 'New Category';?></a>
              ...
              <div class="col-lg-5"> 
                <div class="col-md-12">
                  <select class="form-control" id="location_type">
                    ...
                    <?php
                    foreach($this->db->query("SELECT location_type FROM tbllocation WHERE location_type !='' GROUP BY location_type")->result() as $type) {
                      $sel='';
                      if($row->location_type == $type->location_type){
                        $sel='selected';
                      }
                      echo "<option value='$type->location_type' $sel>".ucwords($type->location_type)."</option>";
                    }
                    ?>
                  </select>
                </div>
