	...
	<div class="container">
		<nav class="submenu">
			<ul>
				...
				<li class="onproject"><a href="<?=base_url('site/restaurant')?>">RESTAURANT/ RETAIL</a></li>
			</ul>
		</nav>
		<div class="contentItem">
			<div class="row">
				<?php 
					foreach($loadrestaurant as $rest) {
					$sql = $this->db->query("SELECT * FROM tblgallery WHERE article_id = '$rest->article_id' limit 1 ")->row();
				?>
				<div class="col-md-3 hover" style="padding-top: 30px;">
					  <img src="<?=base_url().'assets/upload/article/'.$rest->article_id.'_'.$sql->url;?>" alt="" class="image">
					  <div class="middle">
					  	<p><?php echo $rest->article_title;?></p>
					    <a href="<?= site_url('site/articlespecify/'.$rest->article_id);?>"><div class="text">Read More</div></a>
					  </div>
				</div>
					<?php 
						} 
