	...
	<div class="container">
		<nav class="submenu">
			<ul>
				...
				<li class="onproject"><a href="<?=base_url('site/education')?>">EDUCATION</a></li>
		...
		<div class="contentItem">
			<div class="row">
				<?php 
					foreach($loadeducation as $education) {
					$sql = $this->db->query("SELECT * FROM tblgallery WHERE article_id = '$education->article_id' limit 1 ")->row();
				?>
				<div class="col-md-3 hover" style="padding-top: 30px;">
					  <img src="<?=base_url().'assets/upload/article/'.$education->article_id.'_'.$sql->url;?>" alt="" class="image">
					  <div class="middle">
					  	<p><?php echo $education->article_title;?></p>
					    <a href="<?= site_url('site/articlespecify/'.$education->article_id);?>"><div class="text">Read More</div></a>
					  </div>
				</div>
					<?php 
						} 
