<?php
	...
	$tr_search ='<tr class="remove_tag" >
					<td></td>
					<td width="120"><input type="text" name="class_name" id="class_name" class="form-control" onkeyup="search();"></td>
					<td>
					<select name="grade_level" id="grade_level" class="form-control" onchange="search();">
						<option value="">---Select---</option>';
 							foreach ($x as $x) {	
							$tr_search.="<option value='$x->grade_levelid'>$x->grade_level</option>";
						}
		  				$tr_search.='</td>
					<td>
					<select name="grade_label" id="grade_label" class="form-control" onchange="search();">
						<option value="">---Select---</option>';
 							foreach ($a as $a) {	
							$tr_search.="<option value='$a->grade_labelid'>$a->grade_label</option>";
						}
		  				$tr_search.='</td>
	...
	if(count($tdata) >0 ){
		foreach( $tdata as $row_pos ){
			...
			$tr .= '<tr class="no_data" rel="1">
						...
						<td class="pos_2">'.$row_pos['class_name'].'</td>
						<td class="pos_3">'.$row_pos['grade_level'].'</td>
						<td class="pos_4">'.$row_pos['grade_label'].'</td>
						<td class="pos_5"><input '.$checked.' type="checkbox" disabled ></td>
						<td class="pos_6"><input '.$checked1.' type="checkbox" disabled ></td>
						<td class="pos_7"><input '.$checked2.' type="checkbox" disabled ></td>					
						<td class="pos_8">'.$row_pos['created_date'].'</td>
						<td class="pos_9">'.$row_pos['modified_date'].'</td>
						...
						<td>
			              <div class="btn-group">
			                  ...
			                  <ul class="dropdown-menu">
			                      <li ><a title="Contract" rel="'.$row_pos['classid'].'" onclick="edit_pos(event);" role="menuitem" tabindex="-1" href="#">Update</a></li>
			                    <li class="divider"></li>
			                    <li ><a role="menuitem" rel="'.$row_pos['classid'].'" onclick="delete_pos(event);" tabindex="-1" href="#">Delete</a></li>
			                  </ul>
			              </div>
			            </td>   	
	    ...
	    <div id="main_content">
	    	<div class="row result_info">
	      	...
	      	<div class="col-xs-6" style="text-align: right">
		    	...
		    	<span class="top_action_button">
		    		<a href="<?php echo site_url('school/classe/add')?>" >
				      			...
				      			<table class="table" border="0" cellspacing="0" cellpadding="0">
				      				<thead>
				      					<tr>
				      						...
				      						<th class="pos_2 sort" onclick="sort(event);" rel="fee_name">Class Name</th>
				      						<th class="pos_3 sort" onclick="sort(event);" rel="grade_level">Grade Level</th>
				      						<th class="pos_4 sort" onclick="sort(event);" rel="sch_year">Grade Label</th>
				      						<th class="pos_5 sort" onclick="sort(event);" rel="term_start_date">Full Day</th>
				      						<th class="pos_6 sort" onclick="sort(event);" rel="payname">Full Time</th>
				      						<th class="pos_7 sort" onclick="sort(event);" rel="term_start_date">Part Time</th>
				      						<th class="pos_8 sort" onclick="sort(event);" rel="term_start_date">Created Date</th>
				      						<th class="pos_8 sort" onclick="sort(event);" rel="term_start_date">Modified Date</th>
...
<script type="text/javascript">
	...
	function delete_pos(event){
		...
		if(r == true){
			...
			location.href="<?php echo site_url('school/classe/delete');?>/"+sid;
		}
	}
	...
	function edit_pos(event){
		...
		location.href="<?php echo site_url('school/classe/update');?>/"+sid;
	}
	...
	function search(sort){
		...
		var grade_label = $("#grade_label").val();
		var sortpos		= $('#sortquery').val();

 		//alert(sort_num+''+sortpos);

		if(sortpos == '')
			sortpos='ORDER BY classid DESC';
		
		$.ajax({
			type: "POST",
			url:"<?php echo base_url(); ?>index.php/school/classe/search_all",    
			data: {
				'sort_ad':sort,
				'sort':sortpos,
				'sort_num':sort_num,
