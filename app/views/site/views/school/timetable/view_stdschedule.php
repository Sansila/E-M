												...
												foreach ($this->sched->getmorningtimes('AM') as $m) {
													...
													foreach ($this->sched->getdays() as $day) {
														...
														<select class='form-control input-sm subjectid' onchange="getteacher(event);" name='subjectid_<?php echo $m->timeid.'_'.$day->dayid ?>'>
															...
															<?PHP
																if(isset($classid))
																	foreach ($this->sched->getsubject($classid->grade_levelid) as $s) {
																		$selec='';
														...
														<select class='form-control input-sm teacherid' name='teacherid_<?php echo $m->timeid.'_'.$day->dayid ?>'>
															...
															if($subj!='')
																foreach ($this->sched->getteacher($subj,$year->yearid) as $s) {
																	$ishaveclass=$this->sched->ishaveteacher($s->teacher_id,$classid->classid,$year->yearid)->count;
																	$is_bc=$this->sched->isteacherbc($s->teacher_id,$m->timeid,$day->dayid,$year->yearid)->count;
														...
														<select class='form-control input-sm subjectid' onchange="getteacher(event);" name='subjectid_<?php echo $m->timeid.'_'.$day->dayid ?>'>
															...
															<?PHP
																if(isset($classid))
																	foreach ($this->sched->getsubject($classid->grade_levelid) as $s) {
																		$selec='';
																		$ishavesub=$this->sched->ishavesubject($day->dayid,$classid->classid,$s->subjectid,$m->timeid,$year->yearid)->count;
																		if($ishavesub>0){
																			$selec='selected';
																			$subj=$s->subjectid;
																			$teacherid=$ishavesub->teacherid;
...
<script type="text/javascript">
	function selectclass(event){
		...
		var m="<?PHP echo $mg; ?>";
		var p="<?php echo $pg; ?>";
		location.href="<?PHP echo site_url('school/student_sched/scheclass');?>/"+classid+"?m="+m+"&p"+p;
