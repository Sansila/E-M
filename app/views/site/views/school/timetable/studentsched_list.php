	...
	<div class="clearfix" id="main_content_outer">
	    <div id="main_content">
	      <div class="row result_info">
		      	<div class="col-xs-6">
		      		<strong id='title'>Class Schedule List</strong>
		      	</div>
			    	...
			    	<span class="top_action_button">	
		      		...
		      		<span class="top_action_button">
		      			<?php if($this->green->gAction("C")){ ?>
				    		<a href="<?php echo site_url("school/student_sched?m=$m&p=$p")?>" >
		           		...
		           		<div class="table-responsive" id="tab_print">
								...
								foreach($tdata as $row){
									...
									$tr.="<tr>
											...
											if($this->green->gAction("P")){	
											 	$tr.="<a>
											 		<img rel=".$row->classid." onclick='preview(event);' src='".site_url('../assets/images/icons/preview.png')."'/>
											 	</a>";
											}
											if($this->green->gAction("D")){	
											 	$tr.="<a>
											 		<img rel=".$row->classid." onclick='deletes(event);' src='".site_url('../assets/images/icons/delete.png')."'/>
											 	</a>";
											}
		           			...
		           			<table class="table">
		           				...
		           				<thead class='remove_tag'>
		           						<th ></th>	
		           						<th >
		           							<input type='text' value="<?php if(isset($_GET['n'])) echo $_GET['ecode'] ?>" onkeyup="search(event);" id='s_class' class='form-control input-sm'/>
		           						</th>
		           						<th></th>
		           						<th></th>
		           				</thead>
...
<script type="text/javascript">
	...
	function preview(event){
				var classid=jQuery(event.target).attr("rel");
				var yearid=$('#year').val();
				location.href="<?PHP echo site_url('school/student_sched/previewstdsched');?>/"+classid+'/'+yearid;
		}
	function deletes(event){
				var classid=jQuery(event.target).attr("rel");
				var con=confirm("Are you sure to delete this item...!");
				if(con==true)
					location.href="<?PHP echo site_url('school/student_sched/deleteclass');?>/"+classid+"?<?php echo "m=$m&p=$p" ?>";
	...
	function search(event){
		...
		var m=<?php echo $m;?>;
		var p=<?php echo $p;?>;
		var class_name=$('#s_class').val();
		var sort_num=$("#sort_num").val();
		$.ajax({
				url:"<?php echo base_url(); ?>index.php/school/student_sched/search_class",    
