		     						...
		     						<div class="row list">
		     							...
		     							<div class="col-sm-2">
			     							<div class="form_sep" style="margin-top:20px;">
					                            ...
					                            <button class="btn btn-success btn-add" onclick="addnewrow(event);" type="button">
...
$(function(){
		$('#student').keyup(function(e){
			var x=$('#student_id').val();
		    	if(e.keyCode == 13)
		   {
		    	$.ajax({
				type:"post",
				url:"<?php echo base_url(); ?>index.php/library/book_borrow/add",
				data:{
					'x':x
				},
				success:function(d){
	     ...
	     if (e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
	  	...
	  	$('.btn-save').click(function(){
	  		...
	  		$('.list').each(function(){
	  			...
	  			var booklist = $(this).closest('div div').find('.booklist').val();
	...
	function checkbook(x){
		$.ajax({
					type: "POST",
					url:"<?php echo base_url(); ?>index.php/library/book_borrow/checkbook",
					async: false,  
					datatype:"Json", 
					data: {
						'x':x
