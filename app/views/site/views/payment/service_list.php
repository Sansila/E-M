<?php
	$tr_search ='<tr class="remove_tag" >
					<td></td>
					<td><input type="text" name="service_name" id="service_name" class="form-control" onkeyup="search();"></td>
					<td><input type="text" name="cost" id="cost" class="form-control" onkeyup="search();"></td>
	...
	if(count($tdata) >0 ){
		foreach( $tdata as $row_pos ){
			$price_term='No';
			if($row_pos['price_by_term']==1){
				$price_term='Yes';
			}
			$use_quantity='No';
			if($row_pos['use_quantity']==1){
				$use_quantity='Yes';
			}
			$tr .= '<tr class="no_data" rel="1">
						<td class="pos_1">'.($num_pos++).'</td>
						<td class="pos_2">'.$row_pos['service_name'].'</td>
						<td class="pos_3">'.$row_pos['cost'].'</td>
						<td class="pos_4">'.$row_pos['price'].'</td>
						<td class="pos_5">'.$use_quantity.'</td>	
						<td class="pos_5">'.$price_term.'</td>
						...
						<td>
							<div class="btn-group">
		                        ...
		                        <ul class="dropdown-menu">
							      	...
							      	<li ><a title="Delete Contract" rel="'.$row_pos['service_id'].'" onclick="edit_pos(event);" role="menuitem" tabindex="-1" href="#">Update</a></li>
							      	<li class="divider"></li>
							      	<li ><a role="menuitem" rel="'.$row_pos['service_id'].'" onclick="delete_pos(event);" tabindex="-1" href="#">Delete</a></li>
							    </ul>
							</div>
						</td>						
	    ...
	    <div id="main_content">
	    	<div class="row result_info">
	      	...
	      	<div class="col-xs-6" style="text-align: right">
		    	...
		    	<span class="top_action_button">
		    		<a href="<?php echo site_url('payment/service/add')?>" >
				      			...
				      			<table class="table table-bordered table-striped table-hover" border="0" cellspacing="0" cellpadding="0">
				      				<thead>
				      					<tr>
				      						...
				      						<th class="pos_2 sort" onclick="sort(event);" rel="serivename">Service Name</th>
				      						<th class="pos_3 sort" onclick="sort(event);" rel="cost">Cost</th>
				      						<th class="pos_4 sort" onclick="sort(event);" rel="price">Price</th>
				      						<th class="pos_5 sort" onclick="sort(event);" rel="useqty">Use_Quantity</th>
				      						<th class="pos_5 sort" onclick="sort(event);" rel="useqty">Price_By_Term</th>					      						
      ...
      <div class="modal-body">
        <table width="100%" align="center" class="table">
        	<tr>
        		...
        		<th align="center">Pice_By_Term</th>      		
        	</tr>
        	<tr>
...
<script type="text/javascript">
	...
	function search(sort){
		...
		if(sortpos == '')
		sortpos='ORDER BY service_id DESC';
		
		$.ajax({
			type: "POST",
			url:"<?php echo base_url(); ?>index.php/payment/service/search_service",    
			data: {
				'sort_ad':sort,
				'sort':sortpos,
				'sort_num':sort_num,
