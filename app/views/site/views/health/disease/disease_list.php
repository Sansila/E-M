	...
	<div class="clearfix" id="main_content_outer">
	    <div id="main_content">
	      <div class="row result_info">
		      	<div class="col-xs-6">
		      		<strong id='title'>Disease List</strong>
		      	</div>
		           			...
		           			<table class="table">
		           				...
		           				<thead class='remove_tag'>		           					
									<td class='col-xs-1'></td>
							 		<td class='col-xs-2'>
							 			<input type='text' value="<?php if(isset($_GET['des'])) echo $_GET['des']; ?>" onkeyup='search(event);' class='form-control input-sm' id='s_disease_code'/>
							 		</td>
		           					<td class='col-xs-3'>
		           						<input type='text' value="<?php if(isset($_GET['cat'])) echo $_GET['cat']; ?>" onkeyup='search(event);' class='form-control input-sm' id='s_disease'/>
		           					</td>
							 		<td >
							 			<select data-required="true"  class="form-control input-sm parsley-validated" onchange='search(event);' name="s_illness_type" value="" id="s_illness_type" >
								                   <option value="">Select illness</option>
								                   <?php
								                   	foreach ($this->db->get('sch_medi_illness_type')->result() as $row) {
				    ...
				    <div id="main_content">
					    <div class="result_info">
					    	<div class="col-sm-6">
					      	...
					      	<form enctype="multipart/form-data" action="<?php echo base_url()."index.php/health/disease/save?m=$m&p=$p"; ?>"  id="frmdisease" method="POST">
						        <div class="row">
									<div class="col-sm-12">
						            	<div class="panel-body">
						            		<div class="form_sep" id="illness_sep">
								                <label class="req" for="student_num">Illness Type</label>
								                <input type="text" class='hide' name="diseaseid" id="diseaseid" />	
								                <select data-required="true" required data-parsley-required-message="Select Illness Type" min='1' class="form-control parsley-validated" name="illness_type" value="" id="illness_type" >
								                   <option value="">Select illness</option>
								                   <?php
								                   	foreach ($this->db->get('sch_medi_illness_type')->result() as $row) {
								                   		echo "<option value='".$row->illness_typeid."'>".$row->illness_type."</option>";
								            ...
								            <div class="form_sep" id="emp_sep">
							                   <label class="req" for="student_num">Disease Code</label>
							                   <input type="text" required data-parsley-required-message="Enter Disease Code" name="disease_code" id="disease_code"  class="form-control" />	
							                </div>
							                <div class="form_sep" id="emp_sep">
							                   <label class="req" for="student_num">Disease</label>
							                   <input type="text" required data-parsley-required-message="Enter Disease" name="disease" id="disease"  class="form-control" />	
							                </div>
							            </div>
								    </div> 
								</div>
...
<script type="text/javascript">
	...
	function edit(event){
		...
		$.ajax({
                url:"<?php echo base_url(); ?>index.php/health/disease/edit",    
                data: {'diseaseid':diseaseid},
                type:"post",
                dataType:"json",
                async:false,
                success: function(data){
                	$('#disease_code').val(data.disease_code);
                	$('#disease').val(data.disease);
                	$('#diseaseid').val(data.diseaseid);
                	$("#illness_type option[value='"+data.illness_typeid+"']").attr('selected','selected');
            }
        });
	}
	function deletes(event){
			...
			if (r == true) {
			    var diseaseid=$(event.target).attr('rel');
				location.href="<?PHP echo site_url('health/disease/delete');?>/"+diseaseid+"?<?php echo "m=$m&p=$p" ?>";
			} 
	}
	function addnew(){
		$('#frmdisease')[0].reset();
	...
	function search(event){
		...
		var m=<?php echo $m;?>;
		var p=<?php echo $p;?>;
		var disease_code=$("#s_disease_code").val();
		var disease=$("#s_disease").val();
		var illness_typeid=$("#s_illness_type").val();
		var s_num=$('#sort_num').val();
		$.ajax({
				url:"<?php echo base_url(); ?>index.php/health/disease/search",    
				data: {'disease_code':disease_code,
						'disease':disease,
						'illness_typeid':illness_typeid,
						'roleid':roleid,
						'm':m,
						'p':p,
						's_num':s_num
